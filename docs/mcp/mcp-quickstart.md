...
Think of MCP servers as special helpers that give AutoDev extra powers! They let AutoDev do cool things like fetch web pages or work with your files.
...
1. From the AutoDev extension, click the `MCP Server` tab
...
1. AutoDev will detect the change automatically
2. The MCP installer will be downloaded and installed
3. AutoDev will start the MCP installer
4. You'll see the server status in AutoDev's MCP settings UI:
...
Now that you have the MCP installer, you can ask AutoDev to add more servers from:
...
For example, you can ask AutoDev to install the `mcp-server-fetch` package found on the Python Package Index:
...
That's it! 🎉 You've just given AutoDev some awesome new abilities!
...
Now for some good news. While not perfect, you can get AutoDev to do this for you fairly reliably for subsequent server install. Add the following to your "Custom Instructions" in the AutoDev settings (top-right toolbar button):

> When installing MCP servers and editing the autodev_mcp_settings.json, if the server requires use of `npx` as the command, you must copy the "env" entry from the "mcp-installer" entry and add it to the new entry. This is vital to getting the server to work properly when in use.
...
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that AutoDev is seeing.
