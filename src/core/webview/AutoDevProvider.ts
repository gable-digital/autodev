				...
				switch (message.type) {
					...
					case "accountLoginClicked": {
						...
						const authUrl = vscode.Uri.parse(
							`https://app.autodev.bot/auth?state=${encodeURIComponent(nonce)}&callback_url=${encodeURIComponent(`${uriScheme || "vscode"}://gable-digital-solutions.gds-autodev/auth`)}`,
						)
					...
					case "openExtensionSettings": {
						...
						await vscode.commands.executeCommand(
							...
							`@ext:gable-digital-solutions.gds-autodev ${settingsFilter}`.trim(), // trim whitespace if no settings filter
						)
						break
					}
