...
*/
// We don't need to redeclare these types as they are now provided by VSCode's built-in types
...
export class TerminalManager {
	...
	constructor() {
		...
		try {
			disposable = vscode.window.onDidStartTerminalShellExecution?.(async (e: { execution?: { read: () => void } }) => {
	...
	async getOrCreateTerminal(cwd: string): Promise<TerminalInfo> {
		...
		const matchingTerminal = terminals.find((t) => {
			...
			const terminalCwd = t.terminal.shellIntegration?.cwd // one of autodev's commands could have changed the cwd of the terminal
			if (!terminalCwd) {
				return false
			}
